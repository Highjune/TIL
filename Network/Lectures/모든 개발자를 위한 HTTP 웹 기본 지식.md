...
- 분할 전송
    ...
    - 주의) 분할전송할 때는 Content-Lenght를 넣으면 안된다. 왜냐하면 처음에는  Content-Lenght가 얼마인지 예상이 안되고 각각의 용량(청크마다 5바이트)을 따로따로 보낼뿐
- 범위 전송
    - Range, Content-Range
    - /event로 요청 메시지. 
    ```
    ...
    - 예시 상황
        ...
        - 참고
            - 도메인을 찾고 나서 그 안에서 다시 port로 구분
                - aaa.com 의 포트번호 100번에 배달의 민족 주문 서비스(하나의 .jar파일 실행이라고 가정), bbb.com 의 포트번호 100번에 카카오 선물하기 서비스(또 하나의 .jar파일 실행) 이런식으로 따로 연결이 된다는 말. 그렇다면 200.200.200.2에 포트번호 100에는 또 다른 프로그램(서비스)를 연결할 수가 있다.
...
- 캐시가 없을 때(pdf 295)
    ...
    ```
    - 클라이언트가 별(star.jpg)를 받게 됨
    - 이렇게 star.jpg를 보내줄 때 용량이 총 1.1M라 가정한다면
        - HTTP 헤더: 0.1M 으로 가정
        - HTTP 바디: 1.0M 으로 가정
    - 두 번째 요청 (첫 번째 요청과 동일하다)
...
- 캐시 적용했을 때
    ...
    - 캐시 시간이 초과?
        ...
        - 이 때는 당연히 서버를 거치므로 다시 네트워크 다운로드가 발생한다.

## 검증 헤더와 조건부 요청1
- 웹 브라우저가 대부분 실행하고 있는 메커니즘(중요)
- 캐시 시간 초과
...
- 검증 헤더 추가
    ...
    - 그러면 브라우저 캐시에는 캐시, 유효시간뿐 아니라 `데이터 최종 수정일`도 기억해둔다
        ...
        - 데이터 최종 수정일
    - 두 번째 요청
        - 요청시 브라우저 캐시를 먼저 찾았는데 캐시 유효 시간이 초과된 상태.
        - 그런데, 캐시에 Last-Modified 값이 존재한다면, 데이터 최종 수정일을 요청메시지에 넣어서 보냄(`조건부 요청임`). 즉 수정 시간을 서버에 들고 감
        ```
        GET /star.jpg
...
- no-cache vs must-revalidate 
    ...
    - 프록시 캐시 서버 - 원 서버가 연결X 경우 - 순간 네트워크 단절, 원 서버 접근 불가
        
        ![원서버연결안됐을때](https://user-images.githubusercontent.com/57219160/129993215-ea7f0e7d-c99a-4b31-a306-60c9a5b7df01.PNG)
        - no-cache
            1. 캐시 서버 요청 (no-cache + ETag)
